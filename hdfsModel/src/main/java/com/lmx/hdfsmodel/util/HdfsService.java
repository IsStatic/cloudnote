package com.lmx.hdfsmodel.util;
...
public class HdfsService {
    ...
    private static String HDFS_PATH = "hdfs://127.0.0.1:9000/";
    private static String HDFS_USER_NAME = "root";
    ...
    private static Configuration getConfiguration() {
        ...
        configuration.setBoolean("dfs.datanode.use.datanode.hostname", true);
...
//        String path = "hdfs://127.0.0.1:9000/";
//        String username = "root";
//        FileSystem fileSystem = FileSystem.get(getConfiguration());
//        FileSystem fileSystem = FileSystem.get(new URI("hdfs://127.0.0.1:9000/"), getConfiguration(), "root");
        FileSystem fileSystem = FileSystem.get(new URI(HDFS_PATH), getConfiguration(), HDFS_USER_NAME);
        return fileSystem;
    }
...
//    }
     ...
     * @throws Exception 异常
     */
    public static RespBean createFileForString(String path, String fileName, String context) throws Exception {
        if (StringUtils.isEmpty(path) || StringUtils.isEmpty(fileName) || StringUtils.isEmpty(context)) {
...
//            outputStream.writeChars(context);
//            outputStream.writeBytes(context);
     ...
     * @throws Exception 异常
     */
    public static RespBean createFileForStream(String path, String fileName, InputStream input) throws Exception {
     ...
     * @throws Exception 异常
     */
    public static RespBean createFileForBytes(String path, String fileName, byte[] bytes) throws Exception {
        if (StringUtils.isEmpty(path) || StringUtils.isEmpty(fileName) || null == bytes) {
            return RespBean.error(RespBeanEnum.HDFS_CREATE_NULL_ERROR);
        }
        FileSystem fs = getFileSystem();
        Path newPath = new Path(path + "/" + fileName);
     ...
     * @throws Exception
     */
    public static void downloadFile(String path, String targetPath) throws Exception {
        if (StringUtils.isEmpty(path) || StringUtils.isEmpty(targetPath)) {
            return;
        }
        FileSystem fs = getFileSystem();
        // 上传路径
        Path clientPath = new Path(path);
        // 目标路径
        Path serverPath = new Path(targetPath);
...
//        return JsonUtil.toJsonString(jsonStr, clazz);
//    }
